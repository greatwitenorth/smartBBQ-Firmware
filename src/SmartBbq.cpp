...
SmartBbq::SmartBbq(uint8_t probePin1
    ...
    _smartProbes[3] = new SmartProbe(probePin4, ledPin4, ThermistorProbe::ProbeType::IKEA);

    // setup ThermistorProbe
    _thermistorProbe = new ThermistorProbe(9860, 4095);
...
float SmartBbq::getProbeTemp(int probeIndex) {
    if(probeIndex > -1 || probeIndex < PROBENUM) {
        return _thermistorProbe->getTempF(_smartProbes[probeIndex]->getPin(), _smartProbes[probeIndex]->getProbeType(), true);
    } else {
        return _thermistorProbe->getTempF(_smartProbes[_active]->getPin(), _smartProbes[_active]->getProbeType(), true);
    }
}
